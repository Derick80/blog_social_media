...
export default function LikeContainer({ likes, currentUser, postId }: LikeContainerProps) {
  ...
  return (
    <div>
      {userLikedPost ? (
        <button onClick={onClick} className="rounded-md bg-red-500 px-2 py-1 text-white">
          Unlike
        </button>
      ) : (
        <button onClick={onClick} className="rounded-md bg-blue-500 px-2 py-1 text-white">
          Like
        </button>
      )}
    </div>
