...
export default function Home() {
  ...
  return (
    <div className="grid grid-cols-1 grid-rows-1 justify-center p-2 md:grid-cols-6 md:grid-rows-none md:p-4 gap-4 md:gap-8">
      <div className="col-span-full col-start-1 md:col-start-2 md:col-end-6 md:flex-wrap md:row-end-1 justify-center mb-2 md:mb-2">
      ...
      <div> {heroPost?.map((post) => (
            <HeroPost key={post.id} post={post} isLoggedin={isLoggedIn} />
          ))}</div>
      </div>
      <div className="col-span-full gap-4 colums-2 justify-around flex-wrap flex">
          {data?.userPosts?.map((post) => (
            <PostPreview key={post.id} post={post} isLoggedin={isLoggedIn}  currentUser={currentUser}/>
          ))}
        </div>
